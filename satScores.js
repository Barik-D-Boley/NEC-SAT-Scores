...
//The answer to Question 2 is 48915
function questionTwo() {
    let totalScore = 0;
    //Reorders the rows of the array to be in numerical order
    for (i = 0; i < schools.length; i++) {
        schools[i].sort(function(a, b) {
            return a - b;
        });
    }
    //Subtracts the first score from the last score and adds them to the totalScore
    for (i = 0; i < schools.length; i++) {
        let numberOfScores = schools[i].length;
        let currentScore = schools[i][numberOfScores - 1] - schools[i][0];
...
//The answer to Question 3 is 52092.20
function questionThree() {
    //Adds the numbers from every row to undividedAverage, then divides it by the row length
    for (i = 0; i < schools.length; i++) {
        for (j = 0; j < schools[i].length; j++) {
            undividedAverage += Number(schools[i][j]);
        }
        average = undividedAverage/schools[i].length;
    }
...
function questionFour() {
    let schoolAverages = [];
    let orderedAverages = [];
    //Adds the numbers from every row to undividedAverage, then divides it by the row length, and pushes it to schoolAverages
    for (i = 0; i < schools.length; i++) {
        for (j = 0; j < schools[i].length; j++) {
            undividedAverage += Number(schools[i][j]);
        }
    ...
    //Pushes every number from the unordered array into the new array to be ordered
    for (i = 0; i < schoolAverages.length; i++) {
        orderedAverages.push(schoolAverages[i]);
    }
    orderedAverages.sort(function(a, b) {
        return a - b;
    });
    console.log(schoolAverages);

    let largestScore = orderedAverages[orderedAverages.length - 1];
    let smallestScore = orderedAverages[0];

    //Compares each unordered item against every ordered item to see where they lie, then reassigns them at that position
    for (i = 0; i < schoolAverages.length; i++) {
...
function questionFive() {
    ...
    let largestScore = orderedScores[orderedScores.length - 1];
    let smallestScore = orderedScores[0];

    for (i = 0; i < scores.length; i++) {
        for (j = 0; j < scores.length; j++) {
            if (scores[i] == orderedScores[j]) {
                let answer = (scores[i] - smallestScore) / (largestScore - smallestScore);
                scores[i] = (100 * answer).toFixed(0);
            }
        }
    }
    console.log(scores);
